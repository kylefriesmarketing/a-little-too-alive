...
function save(){if(visiting||!world.state.started||saveProblem)return;try{world.state.savedAt=Date.now();localStorage.setItem(SAVE_KEY,JSON.stringify(world.snapshot()));}catch{toast('This browser could not save the garden. Export a postcard to keep it.');}}
...
function launch(guided=false){launched=true;guide=guided?(mix.length?2:1):0;$('intro').hidden=true;if(prefs.sound)audio.set(true).then(updateSound).catch(()=>{});if(saveProblem)toast(saveProblem);updateGuide();if(innerWidth<=900&&guided)$('workbench').classList.add('open');}
$('start-free').onclick=()=>launch(false);$('start-guided').onclick=()=>launch(true);if(world.state.started)launch();
...
function setTool(t){tool=t;view.tool=t;document.querySelectorAll('[data-tool]').forEach(b=>b.classList.toggle('active',b.dataset.tool===t));$('tool-hint').textContent=visiting?'a postcard is a visit. the original world stays with its owner.':HINTS[t];if(innerWidth<=900&&t!=='plant')$('workbench').classList.remove('open');}
document.querySelectorAll('[data-tool]').forEach(b=>b.onclick=()=>setTool(b.dataset.tool));
$('plant-mode').onclick=()=>{setTool('plant');if(innerWidth<=900)$('workbench').classList.remove('open');toast('tap somewhere on the island.');};
function plantAt(x,z){
  ...
  const e=world.plant(mix,x,z,harvested||{});if(!e){toast(world.state.entities.length>=LIMIT?'The island is full. Compost a specimen to make space.':'plant on the island, a little away from its edge.');return;}
...
$('plant-center').onclick=()=>{let p={x:0,z:0};const n=world.state.stats.planted;for(let i=0;i<100;i++){const a=(n+i)*2.399,r=2+((n+i)%8)*.75;p={x:Math.cos(a)*r,z:Math.sin(a)*r};if(!world.state.entities.some(e=>Math.hypot(e.x-p.x,e.z-p.z)<1.2))break;}plantAt(p.x,p.z);};
function inspect(id){selected=id;setTab('inspect');dirty=true;uiStamp='';if(innerWidth<=900)$('observer').classList.add('open');if(guide===4){guide=0;updateGuide();toast('the garden is yours. try eye + heart near brick + heart.');}}
function onWorldTap(p,id){if(!launched)return;
  if(tool==='inspect'||visiting){if(id)inspect(id);return;}
...
function buildSpecimen(e){
  ...
  const type=node('div',undefined,'specimen-kind');type.dataset.field='kind';const title=node('div',e.name,'specimen-name');title.dataset.field='name';const meta=node('div',undefined,'specimen-meta');meta.dataset.field='meta';content.append(type,title,meta);
...
view.sync(world,selected);updateUI();requestAnimationFrame(frame);
