...
export function moveLife(w,e,dt,population){
 ...
 if(e.kind==='home'){e.activity=e.phase==='city'?'carrying a civilization':'walking';}
   ...
   if(target){e.activity='hunting';if(distance(e,target)<1.55){target.health-=dt*(3+e.genes.fang*4);target.fear=clamp(target.fear+dt*.6);e.energy=Math.min(100,e.energy+dt*5);e.activity='feeding';}}
 }
 if(!target&&town&&e.kind==='creature'&&e.age>24&&a.diet!=='predator'){
   e.carrying=e.carrying||0;
   if(e.carrying>=3){target={x:town.x+Math.cos(e.motion)*1.8,z:town.z+Math.sin(e.motion)*1.8};e.activity='bringing food home';if(distance(e,town)<2.7){town.food=(town.food||0)+e.carrying;town.materials=(town.materials||0)+e.carrying*.7;e.carrying=0;e.memory='i helped this place grow.';}}
   else{target=near.find(n=>n.kind==='plant'&&n.age>8);if(target){e.activity='gathering';if(distance(e,target)<1.7){e.carrying+=dt*.7;e.energy=Math.min(100,e.energy+dt);}}}
 }
...
export function seedLivingWorld(w){
 ...
 for(let i=0;i<60;i++){const a=i*2.399,r=3+Math.sqrt(i/60)*24,x=Math.cos(a)*r,z=Math.sin(a)*r;add([['moss','heart'],['moss','eye'],['moss','tear'],['moss','echo']][i%4],x,z,35+i);}
 const families=[];for(let i=0;i<12;i++){const x=-12+(i%4)*3,z=-4+Math.floor(i/4)*3;const genes={flora:.18,mind:.35,love:.7,build:0,fang:.1,water:i%2?.8:.25,heat:.12,song:i%3?.45:.85,motion:.8};families.push(add(['eye','heart','tear'],x,z,45+i,{genes,name:i%2?'Reedstrider':'Crestback'}));}
 for(let i=0;i<4;i++){const p=families[i],q=families[i+4],genes=Object.fromEntries(Object.keys(p.genes).map(k=>[k,(p.genes[k]+q.genes[k])/2]));add(['eye','heart','tear'],p.x+.6,p.z+.6,8,{genes,parentIds:[p.id,q.id],generation:2,name:'Young Crestback',memory:'two lineages, one new possibility.'});}
 for(let i=0;i<3;i++)add(['tooth','ember','eye'],-15-i*2,8+i*2,50,{energy:65,genes:{flora:0,mind:.7,love:.1,build:0,fang:.9,water:.2,heat:.45,song:.2,motion:.8},name:'Cinderjaw'});
 ...
 for(let i=0;i<8;i++)add(['eye','heart','echo'],9+(i%4)*2,1-Math.floor(i/4)*2,38,{society:town.id,name:'Softmouth Forager'});
