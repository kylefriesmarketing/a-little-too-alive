...
const bakedMaterial=new T.MeshStandardMaterial({vertexColors:true,roughness:.85,metalness:.01});
...
function bone(parent,a,b,r,color){const v=new T.Vector3(...b).sub(new T.Vector3(...a));const m=part(parent,'cylinder',color,new T.Vector3(...a).add(new T.Vector3(...b)).multiplyScalar(.5).toArray(),[r,v.length(),r]);m.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),v.normalize());return m;}
function bake(group){const positions=[],normals=[],colors=[];for(const m of [...group.children]){if(!m.isMesh)continue;m.updateMatrix();const geom=m.geometry.index?m.geometry.toNonIndexed():m.geometry.clone();geom.applyMatrix4(m.matrix);const p=geom.attributes.position,n=geom.attributes.normal,c=m.material.color;for(let i=0;i<p.count;i++){positions.push(p.getX(i),p.getY(i),p.getZ(i));normals.push(n.getX(i),n.getY(i),n.getZ(i));colors.push(c.r,c.g,c.b);}geom.dispose();if(m.userData.temporary)m.geometry.dispose();group.remove(m);}if(!positions.length)return;const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(positions,3));geo.setAttribute('normal',new T.Float32BufferAttribute(normals,3));geo.setAttribute('color',new T.Float32BufferAttribute(colors,3));geo.computeBoundingSphere();const mesh=new T.Mesh(geo,bakedMaterial);mesh.castShadow=true;mesh.receiveShadow=true;group.add(mesh);}
...
function leaf(g,origin,length,width,color,angle=0){const verts=[0,0,0,-width,length*.47,0,0,length,.03,width,length*.47,0,0,length*.45,.05];const idx=[0,1,4,1,2,4,2,3,4,3,0,4,4,1,0,4,2,1,4,3,2,4,0,3];const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(verts,3));geo.setIndex(idx);geo.computeVertexNormals();const m=new T.Mesh(geo,material(color));m.position.set(...origin);m.rotation.z=angle;m.userData.temporary=true;g.add(m);}
...
function creature(root,e){const g=e.genes,a=anatomy(g),body=root.userData.body;const base=new T.Color('#aa9756');base.lerp(new T.Color('#338684'),g.water*.8).lerp(new T.Color('#bc6650'),g.heat*.65).lerp(new T.Color('#71957d'),g.flora*.45).lerp(new T.Color('#84618e'),g.song*.28);const skin='#'+base.getHexString(),dark='#'+base.clone().multiplyScalar(.57).getHexString(),belly='#dcc998',crest=g.heat>.4?'#c17640':g.song>.4?'#956ab2':'#cc8e58';
  ...
  part(body,'sphere',skin,[0,h,0],[a.bulk*.62,.48,1.02]);part(body,'sphere',belly,[0,h-.18,.25],[a.bulk*.51,.25,.72]);part(body,'sphere',skin,[0,h+.08,.61],[a.bulk*.52,.46,.47]);
...
export function organism(e){const root=new T.Group();root.userData.entityId=e.id;root.userData.body=new T.Group();root.userData.legs=[];root.userData.height=2.5;root.add(root.userData.body);if(e.kind==='plant')plant(root,e);else if(e.kind==='home')home(root,e);else creature(root,e);return root;}
export function disposeOrganism(root){root.traverse(o=>{if(o.isMesh)o.geometry.dispose();});}
export function signature(e){return (e.carrying>.1?'loaded':'empty')+'|'+e.kind+'|'+(['walking','city'].includes(e.phase)?e.phase:'normal')+'|'+(e.kind==='home'?Math.min(e.residents,12):0)+'|'+Object.values(e.genes).map(n=>n.toFixed(2)).join(',');}
